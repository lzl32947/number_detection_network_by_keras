...
def process_single_input(img, add_noise=False):
    if DataParameter.reshape_only:
        new_img = img.resize((HyperParameter.min_dim, HyperParameter.min_dim), Image.ANTIALIAS)
        x_offset_ratio = 0
        y_offset_ratio = 0
    else:
    ...
    if DataParameter.process_pixel:
        ...
        photo = np.array(new_img, dtype=np.uint8)
